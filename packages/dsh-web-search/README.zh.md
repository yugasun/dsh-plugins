...
给 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的**多后端网页搜索**。向 `ctx.web` 注册百度、豆包、Tavily、Exa；模型侧仍使用官方 `web_search` 工具。
...
安装后请重启 `dsh web`。设置侧栏会出现 **网络搜索** 页。关闭 **自定义搜索** 则继续用 DSH 内置 `web_search`；开启后配置百度 / 豆包 / Tavily / Exa。
...
| Id | 服务 | 密钥 | 说明 |
| --- | --- | --- | --- |
| `baidu` | 千帆智能搜索生成 | `BAIDU_API_KEY` / `QIANFAN_API_KEY` | 使用千帆模型返回总结和网页引用 |
| `doubao` | 火山方舟 Responses + `web_search` | `ARK_API_KEY` / `DOUBAO_API_KEY` | 需要开通联网内容插件，并填写支持该工具的模型 ID |
| `tavily` | Tavily Search | `TAVILY_API_KEY` | 带摘要和可选 answer |
| `exa` | Exa Search | `EXA_API_KEY` | 默认 id 为 `exa`；若同时安装了官方 Exa 包，请改 `exaProviderId` |

密钥可以写在插件设置、DSH 凭据（`TAVILY_API_KEY` 等）或 `~/.dsh/.env`。设置里的密钥不会回显到前端。

「设置 → 网络搜索」里的 **自定义搜索** 开关决定走 DSH 内置 `deepseek-official` 还是本插件。开启后默认 **自动** 按 百度 → 豆包 → Tavily → Exa 选用第一个已配置的。Agent 里 `export TAVILY_API_KEY=…` **不会**进入 Harness 进程。
