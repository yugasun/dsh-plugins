...
| --- | --- | --- | --- |
| `baidu` | [Qianfan web search](https://cloud.baidu.com/doc/qianfan-api/s/Wmbq4z7e5) `POST /v2/ai_search/web_search` (default) or [intelligent search](https://cloud.baidu.com/doc/qianfan-api/s/Hmbu8m06u) `POST /v2/ai_search/chat/completions` | `BAIDU_API_KEY` or `QIANFAN_API_KEY` | Default **ordinary search** returns titles, links, and snippets (faster, no model). Switch to **AI search** in Settings for a model summary; that path is slower, and knowledge-style queries often have **no `references`** — the plugin then recovers `https?://` URLs from the summary when it can. |
